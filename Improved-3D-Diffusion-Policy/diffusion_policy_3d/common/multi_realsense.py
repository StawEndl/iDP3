...
class SingleVisionProcess(Process):
    def __init__(self, device, queue,
        ...
        self.zed = zed
        self.front_queue0 = front_queue0
    ...
    def parse_args(self,init, input_svo_file="", ip_address="", resolution=""):
    ...
    def run(self):
        ...
        # print("000Running Depth Sensing sample ... Press 'Esc' to quit\nPress 's' to save the point cloud")
        ...
        while True:
            ...
            color_frame, depth_frame = self.front_queue0.get()
            if self.enable_pointcloud:
                # Nx6
                point_cloud_frame = self.create_colored_point_cloud(color_frame, depth_frame, 
                            far=self.z_far, near=self.z_near, num_points=self.num_points)
                print(f"point_cloud_frame: {point_cloud_frame.shape}")
            else:
                point_cloud_frame = None
            self.queue.put([color_frame, depth_frame, point_cloud_frame])
            time.sleep(0.05)
        # print(f"???????self.zed.grab(): {self.zed.grab()}")
        # print(f"???????sl.ERROR_CODE.SUCCESS: {sl.ERROR_CODE.SUCCESS}")
        # # while viewer.is_available():
        # while self.zed.grab() == sl.ERROR_CODE.SUCCESS:
        #     image = sl.Mat()
        #     self.zed.retrieve_image(image, sl.VIEW.LEFT, sl.MEM.CPU)
        ...
        #     if self.enable_depth:
        #         depth = sl.Mat()
        #         self.zed.retrieve_measure(depth, sl.MEASURE.DEPTH, sl.MEM.CPU)
...
class MultiRealSense(object):
    def __init__(self, use_front_cam=True, use_right_cam=False,
        ...
        if use_front_cam:
            self.front_process = SingleVisionProcess(None, self.front_queue,
                            ...
                            use_grid_sampling=use_grid_sampling, img_size=img_size, zed=zed, front_queue0=front_queue0,
                            camera_info=camera_info)
        if use_right_cam:
            self.right_process = SingleVisionProcess(None, self.right_queue,
                    enable_rgb=True, enable_depth=True, enable_pointcloud=True, sync_mode=1,
